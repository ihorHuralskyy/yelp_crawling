...
class YelpSelectors:
    class CSSSelectors:
        DIV_WITH_ALL_BUSINESSES = (
            "div.businessName__09f24__EYSZE.css-1jq1ouh a.css-19v1rkv"
        )
        NEXT_PAGE_LINK = (
            "a.next-link.navigation-button__09f24__m9qRz.css-ahgoya::attr(href)"
        )
    ...
    class XPathSelectors:
        ...
        RELATIVE_REVIEW_DATE = (
            ".//div[@class=' css-10n911v']//span[@class=' css-chan6m']/text()"
        )
...
class YelpspiderSpider(scrapy.Spider):
    ...
    custom_settings = {
        "FEEDS": {"businesses_data.json": {"format": "json", "overwrite": True}}
    }
    ...
    def start_requests(self):
        ...
        if not category_name or not location:
            raise Exception(
                "You need to specify both category_name and location atrributes. "
        ...
        url = (
            "https://www."
            + self.MAIN_PAGE
            + f"/search?find_desc={quote(category_name)}&find_loc={quote(location)}"
        )
    ...
    def parse_business(self, response):
        number_of_reviews_text = response.css(
            YelpSelectors.CSSSelectors.NUMBER_OF_REVIEWS_TEXT
        ).get()
        number_of_reviews = (
            int(number_of_reviews_text.split(" ")[0]) if number_of_reviews_text else 0
        )

        yield {
            "business_name": response.css(
                YelpSelectors.CSSSelectors.BUSINESS_NAME
            ).get(),
            "business_rating": response.css(
                YelpSelectors.CSSSelectors.BUSINESS_RATING
            ).get("No rating"),
            "number_of_reviews": number_of_reviews,
            "business_yelp_url": response.url,
            "business_website": response.xpath(
                YelpSelectors.XPathSelectors.BUSINESS_WEBSITE
            ).get("No website"),
            "reviews": self.parse_reviews(
                response.xpath(YelpSelectors.XPathSelectors.LIST_OF_REVIEWS)
            ),
        }
    ...
    def parse_reviews(self, review_divs):
        ...
        for review_div in review_divs:
            review_data = {
                "reviewer_name": review_div.xpath(
                    YelpSelectors.XPathSelectors.RELATIVE_REVIEWER_NAME
                ).get(),
                "reviewer_location": review_div.xpath(
                    YelpSelectors.XPathSelectors.RELATIVE_REVIEWER_LOCATION
                ).get(),
                "review_date": review_div.xpath(
                    YelpSelectors.XPathSelectors.RELATIVE_REVIEW_DATE
                ).get(),
            }
